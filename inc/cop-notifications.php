 ...
 * @param string $thread_title  Thread title.
 * @param string $post_title    Contribution title.
 * @return string
 */
function glandore_cop_build_notification_subject(
	bool $is_reply_mode,
	string $thread_title,
	string $post_title = ''
) : string {
	...
	if ( $is_reply_mode ) {
		if ( '' !== $post_title ) {
			return sprintf( 'CoP comment: %s', $post_title );
		}

		if ( '' !== $thread_title ) {
			return sprintf( 'CoP comment on: %s', $thread_title );
		}
	...
	if ( '' !== $thread_title ) {
		return sprintf( 'CoP discussion: %s', $thread_title );
	}

	return 'CoP discussion';
}
...
function glandore_cop_build_new_discussion_email_body(
	...
	$body .= '<p style="margin:0 0 12px 0; font-size:14px; font-weight:700; line-height:1.35;">' . esc_html( $thread_title ) . '</p>';
	$body .= wp_kses_post( $content_html );
	$body .= '<p><strong>Access discussion:</strong><br>';
...
function glandore_cop_build_contribution_email_body(
	...
	$body .= '<p><strong>Post by ' . esc_html( $sender_name ) . ' on ' . esc_html( $date_string ) . '.</strong></p>';
	$body .= '<p><strong>Access discussion:</strong><br>';
	$body .= '<a href="' . esc_url( $discussion_url ) . '">' . esc_html( $discussion_url ) . '</a></p>';
...
function glandore_cop_send_submission_notifications( array $args ) : int {
	...
	$subject = glandore_cop_build_notification_subject(
		$is_reply_mode,
		$thread_title,
		$post_title
	);
