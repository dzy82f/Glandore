		...
		<div class="page-split-right">
			...
			<?php if ( ! $glandore_is_chain_mode ) : ?>
				...
				<div class="cop-contribution-field">
					<label for="sa-existing" class="cop-contribution-label">
					...
					>
						...
						<?php foreach ( $existing_artefacts as $artefact ) : ?>
							...
							if ( ! empty( $artefact->class_title ) ) {
								$label_parts[] = trim( $artefact->class_title );
							}

							if ( ! empty( $artefact->issue_title ) ) {
								$label_parts[] = trim( $artefact->issue_title );
							}

							$context = ! empty( $label_parts ) ? implode( ' / ', $label_parts ) . ' — ' : '';
