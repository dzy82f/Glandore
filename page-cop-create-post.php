 ...
 *   - direct: private to sender + recipient (+ moderators); NOT included in snapshots
...
// Ensure the CoP DB helpers are available (for contributor table etc).
			...
			if (
				...
				if ( ! $thread_id ) {
					...
					if ( ! $contribution_id ) {
						...
						// 2b) Persist privacy fields (bulletproof).
						$contrib_table = $wpdb->prefix . 'cop_contributions';
						...
						// 4) Email notification for direct posts (link only).
							...
							if ( $recipient && ! empty( $recipient->user_email ) ) {
								...
								$thread_url = add_query_arg(
									array(
										...
										'cop_view_as' => (int) $recipient_user_id, // <- critical
									),
									home_url( '/community-discussions/' )
								);

								$subject = sprintf(
									'Private CoP message from %s',
									$sender ? $sender->display_name : 'a member'
								);

								$body  = "A private Community of Practice message has been sent.\n\n";
								$body .= "View it here:\n{$thread_url}\n";
